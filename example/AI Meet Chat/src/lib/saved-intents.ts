...
/** Newest-first list, filtered to entries whose Intent still exists and whose author isn't blocked. */
export function listSaved(): SavedRecord[] {
  const intent = (id: string) => getIntentById(id);
  return read()
    .filter((r) => {
      const it = intent(r.intentId);
