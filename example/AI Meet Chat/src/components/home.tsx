...
import { useConnections, hasUnseenIn, useSession, useSessionActions } from "@/data/hooks";
...
export function Home() {
  ...
  const unseen = hasUnseenIn(useConnections(true).data);
  ...
  async function submit() {
    ...
    if (target === "sidebyside") {
      const s = await createSession("do_something", body, EMPTY_SIDE);
      void navigate({ to: "/side-by-side", search: { session: s.id, chatWith: "" } });
      return;
    }
    const s = await createSession("introduce", body, EMPTY_MATCHMAKER);
