...
// Reads now go through the data layer (@/data/hooks), so swapping the local
// adapter for a real backend does not touch this component.
...
export function SavedTrigger({
  ...
  const saved: SavedRecord[] = useSavedWishes().data;
  const people: SavedPersonRecord[] = useSavedPeople().data;
  const { removeWish, removePerson } = useSavedActions();
  const intentById = useIntentLookup();
  const personById = usePeopleLookup();
  const sessionById = useSessionLookup();
        ...
        >
                  ...
                  return (
                    <li key={rec.personId} className="rounded-lg border border-border bg-card p-4">
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        ...
                        <button
                          onClick={() => void removePerson(rec.personId)}
          ...
          <TabsContent value="wishes" className="flex-1 overflow-y-auto px-6 py-4 mt-0">
            {!hydrated ? (
              ...
              <ul className="space-y-3">
                {saved.map((rec) => {
                  const intent = intentById(rec.intentId) as Intent | null;
                  if (!intent) return null;
                  const person = personById(intent.ownerId);
                  ...
                  const session = sessionById(rec.sessionId);
                  ...
                  return (
                    <li key={rec.intentId} className="rounded-lg border border-border bg-card p-4">
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        ...
                        <button
                          onClick={() => void removeWish(rec.intentId)}
                          aria-label={t("saved.remove")}
                          className="inline-flex items-center gap-1.5 px-3 py-2 rounded-md border border-border text-[12.5px] text-foreground/85 hover:bg-secondary transition-colors"
                        >
                          <X className="w-3 h-3" />
...
export function useIsSaved(intentId: string | null | undefined): boolean {
  const saved = useSavedWishes().data;
