...
export function IntroCanvas({ state, sessionId, onPassAndNext, onSeeNextPerson }: Props) {
  ...
  const person = state.currentPersonId ? personById(state.currentPersonId) : null;
  // Everything below reads through the data layer (@/data/hooks) instead of
  // touching browser storage directly.
  const conn = useConnection(person?.id ?? "").data;
  const { sayHello } = useConnectionActions();
  const savedPeople = useSavedPeople().data;
  const { togglePerson, removePerson } = useSavedActions();
  const [composing, setComposing] = useState(false);
  const [draftPicked, setDraftPicked] = useState<string | null>(null);
  const [draftReply, setDraftReply] = useState("");
  const saved = !!person && savedPeople.some((r) => r.personId === person.id);
  const [profileOpen, setProfileOpen] = useState(false);
  // My own profile, used to work out why this person might fit.
  const storedProfile = useProfile().data;
  const [hydratedProfile, setHydratedProfile] = useState(false);
  useEffect(() => {
    setHydratedProfile(true);
  }, []);
  const myProfile = hydratedProfile ? storedProfile : null;
  ...
  // Auto-remove from Saved once a real connection begins — Save is a
  // pre-decision holding pattern only.
  useEffect(() => {
    if (person && conn && conn.status !== "faded" && saved) {
      void removePerson(person.id);
    }
    // Only re-run when the person or the connection status changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [person?.id, conn?.status, saved]);
  ...
  useEffect(() => {
    ...
    if (person) {
      ...
      const resuming = resumeId !== null && isVitalsComplete(storedProfile);
      const d =
  ...
  function requestSayHello(opts?: { pickedMomentId?: string | null; draftReply?: string }) {
    if (!isVitalsComplete(storedProfile)) {
      const nextDraft = {
        composing: true,
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  onClick={() => {
                    if (!person) return;
                    void togglePerson(person.id, sessionId);
                  }}
                  aria-pressed={saved}
                  className={[
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between -mx-2">
                ...
                <button
                  ...
                  onClick={() => {
                    if (!person) return;
                    if (!saved) void togglePerson(person.id, sessionId);
                    handleCancel();
                  }}
