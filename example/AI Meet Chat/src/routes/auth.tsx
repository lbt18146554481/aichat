...
import { AuthError, type AuthProvider } from "@/lib/auth";
import { useAuthActions, useSession, useValidateInvite } from "@/data/hooks";
...
function AuthPage() {
  ...
  async function handleVerifyInvite(e: FormEvent) {
    ...
    setPending("verify");
    try {
      const ok = await validateInvite.mutateAsync(code);
      if (!ok) {
  ...
  async function handleProvider(provider: AuthProvider) {
    ...
    try {
      if (mode === "signup") {
        ...
        await signUp(provider, verifiedCode);
