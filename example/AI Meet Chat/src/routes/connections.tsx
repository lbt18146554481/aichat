...
import { useConnections, usePeopleLookup } from "@/data/hooks";
...
function ConnectionsPage() {
  ...
  // Reads flow through the data layer; `bootstrap` replaces the old
  // rehydrate() call so pending timers resume exactly as before.
  const connections = useConnections(true);
  const items = useMemo<Connection[]>(
    () => [...connections.data].sort((a, b) => activityAt(b) - activityAt(a)),
    [connections.data],
  );
  // `loaded` separates "still fetching" from "genuinely empty", so the list
  // can show skeletons first and the empty state only once.
  ...
  useEffect(() => {
    if (!ready) return;
    if (!connections.isFetching) setLoaded(true);
  }, [ready, connections.isFetching, connections.data]);
...
function Row({
  ...
  const person = personById(conn.personId);
