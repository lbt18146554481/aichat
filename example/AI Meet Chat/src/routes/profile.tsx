...
import { isProfileComplete, profileProgress, type Profile } from "@/lib/profile";
...
function ProfilePage() {
  ...
  // Stored profile now comes from the data layer; local state still tracks
  // in-progress edits streamed up from the form.
  const stored = useProfile().data;
  const [currentProfile, setCurrentProfile] = useState<Profile>(stored);

  const isWelcome = search.welcome === 1 && !returnTo;

  useEffect(() => {
    setCurrentProfile(stored);
    setProgress(profileProgress(stored));
  }, [stored]);
