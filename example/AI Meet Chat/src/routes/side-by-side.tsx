...
import { findMatch, findNearMisses, type Intent } from "@/lib/intents";
import { useAgentMemory, useIntentLookup, usePeopleLookup, useProfile } from "@/data/hooks";
...
type IntentLookup = (id: string | null | undefined) => Intent | null;

function summarize(intentId: string | null, t: TFunction, intentById: IntentLookup): string {
  if (!intentId) return "";
...
function narrate(
  ...
  if (view === "match") {
    ...
    return msg(
      "assistant",
      truncated +
        t("intent.narrate_matched", { summary: summarize(state.myIntentId, t, intentById) }),
      matchChips(t),
    );
  }

  if (view === "nomatch") {
    const mine = intentById(state.myIntentId);
    if (!mine) return null;
    ...
    if (refineChips.length === 0) {
      return msg(
        "assistant",
        truncated +
          t("intent.narrate_nomatch_wait", { summary: summarize(state.myIntentId, t, intentById) }),
        chips,
      );
    }
    ...
    return msg(
      ...
      truncated +
        t("intent.narrate_nomatch_ask", {
          summary: summarize(state.myIntentId, t, intentById),
...
function SideBySidePage() {
  ...
  // Reads go through the data layer. A ref keeps the latest lookup available
  // inside the deferred setState callbacks below.
  const intentById = useIntentLookup();
  const intentByIdRef = useRef(intentById);
  const personById = usePeopleLookup();
  const personByIdRef = useRef(personById);
  const { memory, rememberTrait } = useAgentMemory();
  const memoryRef = useRef(memory);
  const myCity = useProfile().data.city;

  useEffect(() => {
    intentByIdRef.current = intentById;
  }, [intentById]);

  useEffect(() => {
    personByIdRef.current = personById;
  }, [personById]);
  ...
  // previous activity, lead with it. Otherwise the plain opener. Waits until
  // the memory repo has answered so the prefix is never dropped by a race.
  useEffect(() => {
    if (!hydrated || memory === null) return;
    if (state.messages.length === 0) {
      const trait = memory.preferredTraits[0] ?? null;
      const opening = trait
  ...
  function actWith(mutate: (s: SideState) => SideState, userText?: string) {
    ...
    window.setTimeout(() => {
      setState((s) => {
        ...
        const narr = narrate(next, s, t, intentByIdRef.current);
  ...
  function respondAboutPerson(userText: string) {
    ...
    window.setTimeout(() => {
      setState((s) => {
        const other = intentByIdRef.current(s.matchIntentId);
        const person = other ? personByIdRef.current(other.ownerId) : null;
  ...
  function startNewActivity(userText?: string) {
    ...
    window.setTimeout(() => {
      setState((s) => {
        ...
        const trait = memoryRef.current?.preferredTraits[0] ?? null;
        const line = trait
