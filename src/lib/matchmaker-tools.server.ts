...
/** Tool-phase system prompt (no JSON reply yet). Always Chinese — planning only. */
export function matchmakerToolSystem(state: MatchmakerToolState): string {
  const loc = formatPlaceList(parsePlaceList(state.hardFilters.cities), "zh-CN");
  const genderLine =
    state.hardFilters.genders.length > 0
      ? state.hardFilters.genders.map((g) => genderLabel(g, "zh-CN")).join("、")
      : "不限";
  const queueLine = `排序队列：${state.rankedQueueLength > 0 ? `${state.rankedQueueLength} 人` : "无"}`;
  return `你是 Matchmaker 的工具规划助手。根据用户这句话，在需要时调用工具：
